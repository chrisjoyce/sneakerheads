...
const cacheLayer: CacheLayer = new CacheLayer({ redis: {} });
...
async function StartSoldSearch(): Promise<any> {
  ...
  let lastScrapedID: number = cacheLayer.LastScrapedID;
  ...
  while (!finishedParsing && currentPage < 5) {
    ...
    let sold_shoes: any[] = await page.$$eval('.s-item', (links: Element[]) => {
    ...
    foundLastItemIndex = sold_shoes.findIndex(shoe => {
      console.log(`${shoe.id} - ${lastScrapedID}`);
      return shoe.id === lastScrapedID;
    });

    console.log(`Last Item: ${lastScrapedID} was found: ${foundLastItemIndex}`);
    ...
    sold_shoes.forEach(item => {
      client.sadd(unprocessedItems, JSON.stringify(item));
    });
